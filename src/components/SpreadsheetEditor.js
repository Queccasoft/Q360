...
import { Container, Button, ButtonGroup, Table, Form, Dropdown, DropdownButton, Tab, Tabs, Modal } from 'react-bootstrap';
...
// --- COMPONENT ---
const SpreadsheetEditor = ({ fileHandle, onSave, onDownload }) => {
  // --- CONFIG ---
...
const initialChartData = [
  { 
      ...
      type: "pie",
      data: [
          { label: "CPIM", value: 2, color: "rgba(255, 99, 132, 0.8)", gradient:"none" },
          { label: "TMC", value: 5, color: "rgba(75, 192, 192, 0.8)", gradient:"none" },
          { label: "BJP", value: 8, color: "rgba(255, 205, 86, 0.8)", gradient:"none" }
      ] 
  }
];
...
const [chartTitle, setChartTitle] = useState('My New Chart');
  ...
  // l.download = `${projectName || 'Spreadsheet'}.ods`;
  // l.click();
  if (mode === 'save' && onSave) {
    onSave(b, projectName || 'Spreadsheet', '.ods');
  } else if (onDownload) {
    onDownload(b, projectName || 'Spreadsheet', '.ods');
  }
};
...
const exportToQSheet = async (mode = 'save') => {
  ...
  try {
    ...
    // const fileName = `${projectName.replace(/[^a-z0-9]/gi, '_')}.qsheet`;
...
return (
    <Container fluid className="p-0 bg-light vh-100 d-flex flex-column" onMouseUp={handleMouseUp}>
      {/* TOOLBAR */}
      <input 
      type="text" 
    ...
    />
      <div className="bg-white border-bottom p-2 d-flex flex-wrap gap-2 align-items-center">
        <Button variant="outline-danger" onClick={handleNewProject} title='Create New Project' size='sm'>New</Button>
        <ButtonGroup size='sm'>
    {/* 1. SAVE BUTTONS (Calls onSave via FileManager) */}
    <Button variant="success" onClick={() => exportToOds('save')} title='Save to Folder (ODS)'>Save ODS</Button>
    <Button variant="success" onClick={() => exportToQSheet('save')} title='Save to Folder (QSheet)'>Save QSheet</Button>
    
    {/* 2. DOWNLOAD BUTTONS (Calls onDownload via FileManager) */}
    <Button variant="outline-success" onClick={() => exportToOds('download')} title='Download File'>Down ODS</Button>
    <Button variant="outline-success" onClick={() => exportToQSheet('download')} title='Download File'>Down QSheet</Button>
    
    {/* 3. IMPORT BUTTONS (Independent) */}
    <Button variant="outline-primary" onClick={() => importODSInputRef.current.click()} title='Import ODS'>Imp ODS</Button>
    <Button variant="outline-primary" onClick={() => importQSHEETInputRef.current.click()} title='Import QSheet'>Imp QSheet</Button>

    {/* Hidden Inputs for Import Buttons */}
    <input type="file" hidden ref={importODSInputRef} onChange={(e) => handleImport(e)} accept=".ods" />
    <input type="file" hidden ref={importQSHEETInputRef} onChange={(e) => handleQSheetImport(e)} accept=".qsheet" />
        ...
        <Button variant="outline-danger" size="sm" onClick={exportToPDF} title="Export PDF" className="bx bxs-file-pdf">PDF</Button>
        </ButtonGroup>

        <div className="vr mx-1"></div>
        <ButtonGroup className="ms-2">
        <Button variant="outline-info" size="sm" onClick={() => setShowRuleModal(true)}>Filter</Button>
        <Button size="sm" variant="outline-secondary" title="Clear Filters" onClick={() => clearAllFilters()} className='bx bx-hide'></Button>
        </ButtonGroup>

        <DropdownButton size="sm" variant="outline-secondary" title="Font">
          {fontFaces.map(f => <Dropdown.Item key={f} onClick={() => applyStyle('fontName', f)}>{f}</Dropdown.Item>)}
        </DropdownButton>
        <DropdownButton size="sm" variant="outline-secondary" title="Size">
          {fontSizes.map(s => <Dropdown.Item key={s} onClick={() => applyStyle('fontSize', s)}>{s}</Dropdown.Item>)}
        </DropdownButton>

      <ButtonGroup className="ms-2">
        <Button variant="outline-secondary" size="sm" onClick={() => handleCopy(true)}>Cut</Button>
        <Button variant="outline-secondary" size="sm" onClick={() => handleCopy(false)}>Copy</Button>
        
        <Dropdown as={ButtonGroup}>
          <Button variant="outline-primary" size="sm" onClick={() => executePaste('all')} disabled={!clipboard}>Paste</Button>
          <Dropdown.Toggle split variant="outline-primary" size="sm" id="dropdown-split-basic" disabled={!clipboard} />
          <Dropdown.Menu>
            <Dropdown.Item onClick={() => setShowPasteModal(true)}>Paste Special</Dropdown.Item>
          </Dropdown.Menu>
        </Dropdown>
        <Button variant="outline-danger" size="sm" onClick={handleClearAll} title="Clear Selection">Clear</Button>
      </ButtonGroup>

      <DropdownButton variant="outline-dark" size="sm" title="🪄 Auto-Fill" className="ms-2">
        <Dropdown.Item onClick={() => handleAutoFill('down')}>Fill Down</Dropdown.Item>
        <Dropdown.Item onClick={() => handleAutoFill('right')}>Fill Right</Dropdown.Item>
        <Dropdown.Divider />
        <Dropdown.Header>Series Type</Dropdown.Header>
        <Dropdown.Item onClick={() => alert("Auto-detected by content (Date/Number)")}>
          Format: Auto-Detect
        </Dropdown.Item>
      </DropdownButton>
       
        <ButtonGroup size="sm">
          <Button variant="outline-secondary" onMouseDown={(e) => {e.preventDefault(); applyStyle('bold');}}><b>B</b></Button>
          <Button variant="outline-secondary" onMouseDown={(e) => {e.preventDefault(); applyStyle('italic');}}><i>I</i></Button>
          <Button variant="outline-secondary" onMouseDown={(e) => {e.preventDefault(); applyStyle('underline');}}><u>U</u></Button>
          <Button variant="outline-secondary" onMouseDown={(e) => {e.preventDefault(); applyStyle('strikethrough');}}><del>S</del></Button>
        </ButtonGroup>

        <DropdownButton size="sm" variant="outline-secondary" title="Align">
           <Dropdown.Item onClick={() => applyStyle('justifyLeft')}>Left</Dropdown.Item>
           <Dropdown.Item onClick={() => applyStyle('justifyCenter')}>Center</Dropdown.Item>
           <Dropdown.Item onClick={() => applyStyle('justifyRight')}>Right</Dropdown.Item>
           <Dropdown.Divider />
           <Dropdown.Item onClick={() => applyStyle('verticalAlign', 'top')}>Top</Dropdown.Item>
           <Dropdown.Item onClick={() => applyStyle('verticalAlign', 'middle')}>Middle</Dropdown.Item>
           <Dropdown.Item onClick={() => applyStyle('verticalAlign', 'bottom')}>Bottom</Dropdown.Item>
        </DropdownButton>

        {/* BORDER CONTROLS */}
        <div className="d-flex align-items-center gap-1 border rounded p-1">
           <Form.Select size="sm" style={{width: 80}} value={borderMode} onChange={(e) => setBorderMode(e.target.value)}>
           ...
           <Form.Control type="color" size="sm" style={{width: 30}} value={borderColorPicker} onChange={(e) => setBorderColorPicker(e.target.value)} title="Border Color" />
           <Button size="sm" variant="outline-dark" onClick={applyBorder}>Apply</Button>
        </div>

        <div className="d-flex align-items-center gap-1">
           <Form.Control type="color" size="sm" style={{width: 30}} onChange={(e) => applyStyle('foreColor', e.target.value)} title="Text Color" />
           <Form.Control type="color" size="sm" style={{width: 30}} onChange={(e) => applyStyle('hiliteColor', e.target.value)} title="Cell Color" />
        </div>
        ...
        >
          ...
          <Dropdown.Item onClick={() => insertFormula('SUM')}>SUM (Total)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('AVERAGE')}>AVERAGE (Mean)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('COUNT')}>COUNT (Numbers)</Dropdown.Item>
          
          <Dropdown.Divider />
          <Dropdown.Header>Math & Stats</Dropdown.Header>
          <Dropdown.Item onClick={() => insertFormula('MIN')}>MIN (Smallest)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('MAX')}>MAX (Largest)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('PRODUCT')}>PRODUCT (Multiply)</Dropdown.Item>
          
          <Dropdown.Divider />
          <Dropdown.Header>Text</Dropdown.Header>
          <Dropdown.Item onClick={() => insertFormula('UPPER')}>UPPER (Uppercase)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('LOWER')}>LOWER (Lowercase)</Dropdown.Item>
          <Dropdown.Item onClick={() => insertFormula('CONCAT')}>CONCAT (Join)</Dropdown.Item>
        </DropdownButton>

        <DropdownButton variant="outline-primary" size="sm"  title="Cells" id="insert-cell-dropdown">
          <Dropdown.Item onClick={() => handleCellShift('insert-right')}>Add & Shift Right ⮕</Dropdown.Item>
          <Dropdown.Item onClick={() => handleCellShift('insert-down')}>Add & Shift Down ⬇</Dropdown.Item>
          <Dropdown.Item onClick={() => handleCellShift('delete-left')}>Delete & Shift Left ⬅</Dropdown.Item>
          <Dropdown.Item onClick={() => handleCellShift('delete-up')}>Delete & Shift Up ⬆</Dropdown.Item>
        </DropdownButton>

        <DropdownButton size="sm" variant="primary" title="Rows">
          <Dropdown.Item onClick={() => modifyStructure('row', 'add', 'above')}>Insert Above</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('row', 'add', 'below')}>Insert Below</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('row', 'add', 'end')}>Add to End</Dropdown.Item>
          <Dropdown.Divider />
          <Dropdown.Item onClick={() => modifyStructure('row', 'del', 'at')} className="text-danger">Delete Selected</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('row', 'del', 'end')} className="text-danger">Delete Last</Dropdown.Item>
          <Dropdown.Item onClick={() => duplicateStructure('row')} className="text-primary">Duplicate Selected Row</Dropdown.Item>
          <Dropdown.Item onClick={swapRows} className="text-info">Swap Rows...</Dropdown.Item>
          <Dropdown.Divider />
          <Dropdown.Header>Sort Whole Sheet by this Row</Dropdown.Header>
          <Dropdown.Item onClick={() => sortStructure('row', 'az')}>A → Z</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('row', 'za')}>Z → A</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('row', 'bg')}>By Background Color</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('row', 'text')}>By Text Color</Dropdown.Item>
        </DropdownButton>

        <DropdownButton size="sm" variant="primary" title="Cols">
          <Dropdown.Item onClick={() => modifyStructure('col', 'add', 'left')}>Insert Left</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('col', 'add', 'right')}>Insert Right</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('col', 'add', 'end')}>Add to End</Dropdown.Item>
          <Dropdown.Divider />
          <Dropdown.Item onClick={() => modifyStructure('col', 'del', 'at')} className="text-danger">Delete Selected</Dropdown.Item>
          <Dropdown.Item onClick={() => modifyStructure('col', 'del', 'end')} className="text-danger">Delete Last</Dropdown.Item>
          <Dropdown.Item onClick={() => duplicateStructure('col')} className="text-primary">Duplicate Selected Column</Dropdown.Item>
          <Dropdown.Item onClick={swapCols} className="text-info">Swap Columns...</Dropdown.Item>
          <Dropdown.Divider />
          <Dropdown.Header>Sort Whole Sheet by this Column</Dropdown.Header>
          <Dropdown.Item onClick={() => sortStructure('col', 'az')}>A → Z</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('col', 'za')}>Z → A</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('col', 'bg')}>By Background Color</Dropdown.Item>
          <Dropdown.Item onClick={() => sortStructure('col', 'text')}>By Text Color</Dropdown.Item>
          <Dropdown.Item onClick={() => applyQuickFilter()}>Filter</Dropdown.Item>
        </DropdownButton>

        <ButtonGroup size="sm">
          <Button variant="outline-info" onClick={() => resizeDimension('col', -0.5)}>W-</Button>
          <Button variant="outline-info" onClick={() => resizeDimension('col', 0.5)}>W+</Button>
          <Button variant="outline-info" onClick={() => resizeDimension('row', -0.2)}>H-</Button>
          <Button variant="outline-info" onClick={() => resizeDimension('row', 0.2)}>H+</Button>
        </ButtonGroup>

        <Button variant="outline-warning" size="sm" onClick={toggleMerge} title='Merge selection'>Merge</Button>
        <Button variant="outline-primary" size="sm" onClick={() => fileInputRef.current.click()} title='Insert Image'>Img</Button>
        <Button variant="outline-success" size="sm" className="ms-2" onClick={() => setShowChartModal(true)} title='Insert Chart'>📊 Chart</Button>
        <Button variant="outline-info" size="sm" onClick={() => setShowFindModal(true)}>Search & Replace</Button>
      ...
      {/* EDITABLE GRID */}
      <div className="flex-grow-1 overflow-auto bg-white" style={{ userSelect: 'none' }}>
